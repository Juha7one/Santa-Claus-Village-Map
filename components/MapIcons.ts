...
export const placeMarkerIcon = (categoryKey: string, selected: boolean, isLoved: boolean, placeId: string, subCategory?: string, originalId?: string, status?: 'Open' | 'Closed') => {
    ...
    const closedLine = isClosed ? '<div class="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden rounded-full"><div class="w-[140%] h-[2.5px] bg-white rotate-45 shadow-[0_0_2px_rgba(0,0,0,0.5)]"></div></div>' : '';
    const closedLinePin = isClosed ? '<div class="absolute top-[2px] left-0 w-8 h-8 flex items-center justify-center pointer-events-none overflow-hidden"><div class="w-[100%] h-[2.5px] bg-white rotate-45 shadow-[0_0_2px_rgba(0,0,0,0.5)]"></div></div>' : '';
    ...
    if (isLoved) {
        ...
        return new L.DivIcon({
            html: `<div class="w-8 h-8 rounded-full flex items-center justify-center shadow-md border-2 border-white relative" style="background-color: ${baseColor};">${heartSvg}${closedLine}</div>`,
